...
// Dynamically load the editor on client only (prevents SSR hydration issues)
const TiptapEditor = dynamic(() => import("@/components/TiptapEditor"), { ssr: false });

// Define the shape of the user object for better type safety
interface User {
  id: string;
  name: string;
}
...
const AdminDashboard = () => {
  ...
  const [formData, setFormData] = useState({
    title: "",
    body: "", // will hold HTML from Tiptap
    category: "",
    author: "",
    country: "",
    degree: "",
  ...
  const [isMounted, setIsMounted] = useState(false); // ➕ ensure we only render editor after mount
  const router = useRouter();

  // Hydration-safe mount
  useEffect(() => setIsMounted(true), []);

  // Memoize the handleAuthError function to prevent unnecessary re-creations
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Optional: prevent empty HTML like <p></p>
    ...
    try {
      ...
      formDataToSend.append('body', formData.body); // HTML from Tiptap
      ...
      if (response.ok) {
        toast.success("Blog post created successfully! Reloading page to reflect changes.");
  ...
  // Add a check to prevent rendering the dashboard before data is available
                  ...
                  {isMounted && (
                    <div className="mt-2">
                      <TiptapEditor
                        ...
                        placeholder="Write your post content here. Use the toolbar to bold, italicize, or add links (e.g., WhatsApp)..."
                      />
                    </div>
                  )}
                </div>

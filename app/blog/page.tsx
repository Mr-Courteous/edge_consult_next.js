...
import { Calendar, User, Tag, ArrowRight, MessageCircle, Twitter, Facebook, Linkedin, Copy, Share2, Check, Trash2, Loader2 } from "lucide-react";
  ...
  const SocialShareButtons = ({ title, url }) => {
    ...
    return (
      <div className="flex items-center gap-4">
        <a href={`https://twitter.com/intent/tweet?url=${encodedUrl}&text=${encodedTitle}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-violet-500 hover:text-sky-400 hover:bg-violet-100 transition-colors duration-200">
          <Twitter size={20} />
        </a>
        <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-violet-500 hover:text-blue-600 hover:bg-violet-100 transition-colors duration-200">
          <Facebook size={20} />
        </a>
        <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodedUrl}`} target="_blank" rel="noopener noreferrer" className="p-1 rounded-full text-violet-500 hover:text-blue-700 hover:bg-violet-100 transition-colors duration-200">
          <Linkedin size={20} />
        </a>
    ...
    <Layout>
      ...
      <section className="py-4 bg-gradient-to-r from-violet-100 via-fuchsia-50 to-indigo-100 border-b border-t sticky top-0 z-20 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 md:px-10 flex flex-wrap gap-3 justify-center">
          {categories.map(category => (
            <Button key={category} variant={selectedCategory === category ? "default" : "outline"} onClick={() => setSelectedCategory(category)} className={`font-orbitron text-sm px-5 py-2 rounded-full transition-all duration-200 shadow ${selectedCategory === category ? "bg-gradient-to-r from-fuchsia-500 via-violet-500 to-indigo-500 text-white" : "bg-white text-violet-700 border-violet-200 hover:bg-violet-50"}`}>
      ...
      <section className="py-16 bg-gradient-to-b from-white via-violet-50 to-fuchsia-50">
        <div className="max-w-7xl mx-auto px-4 md:px-10">
          <div className="mb-10 text-center">
            ...
            <p className="font-inter text-violet-700/80">Stay updated with our latest insights and industry analysis</p>
          </div>
          <div className="flex justify-center mb-8">
            <Input type="text" placeholder="Search posts..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full max-w-md font-inter border-violet-200 focus:border-violet-500 transition-colors duration-200" />
          </div>

          {loading ? (
            <div className="text-center py-12"><p className="font-orbitron text-violet-700">Loading posts...</p></div>
          ) : filteredAndSearchedPosts.length === 0 ? (
            <div className="text-center py-12"><p className="font-orbitron text-violet-700">No posts available yet.</p></div>
          ) : (
                ...
                <Card key={post._id} className="overflow-hidden border-none bg-gradient-to-br from-white via-violet-100 to-fuchsia-100 shadow-xl rounded-2xl hover:scale-105 transition-all duration-200">
                  ...
                  <CardContent className="pt-0">
                    ...
                    <div className="flex items-center gap-2 mt-4 mb-4">
                      ...
                      <SocialShareButtons title={post.title} url={`${window.location.origin}/post/${post._id}`} />
                      {user && user.role === 'admin' && (
                        <Button variant="ghost" size="icon" className="text-fuchsia-600 hover:bg-fuchsia-100 hover:text-fuchsia-700 transition-colors duration-200"
                          onClick={() => { setPostToDeleteId(post._id); setShowDeleteModal(true); }}
                          disabled={deleting}>
                          {deleting && postToDeleteId === post._id ? <Loader2 className="h-5 w-5 animate-spin" /> : <Trash2 className="h-5 w-5" />}
                        </Button>
                      )}
                    </div>
                    <Button variant="ghost" className="w-full py-3 font-orbitron text-violet-700 hover:text-fuchsia-700 hover:bg-violet-100 transition-colors duration-200" onClick={() => router.push(`/post/${post._id}`)}>
      ...
      <section className="py-16 bg-gradient-to-r from-violet-100 via-fuchsia-50 to-indigo-100 rounded-xl shadow-inner my-16">
        <div className="max-w-3xl mx-auto text-center px-4 md:px-10">
          ...
          <p className="font-inter text-violet-700/80 mb-6">Subscribe to our newsletter for the latest blog posts, industry news, and exclusive insights directly in your inbox.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Input type="email" placeholder="Enter your email" value={newsletterEmail} onChange={e => setNewsletterEmail(e.target.value)} className="w-full sm:w-auto flex-grow font-inter" />
      ...
      <ConfirmationModal isOpen={showDeleteModal} onCancel={() => setShowDeleteModal(false)} onConfirm={handleDeletePost} message="Are you sure you want to delete this post? This action is permanent." isLoading={deleting} />
    </Layout>
  );
};
